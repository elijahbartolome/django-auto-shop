...
class AppointmentRequest(models.Model):
    ...
    ar_id = models.CharField(max_length=100, blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.ar_id is None:
            self.ar_id = f"{get_timestamp()}{self.service.id}{generate_random_id()}"
        if self.start_time == self.end_time:
            raise ValidationError(_("Start time and end time cannot be the same"))
...
class Appointment(models.Model):
    ...
    appoint_id = models.CharField(max_length=100, blank=True, null=True)
    inquiry = models.ForeignKey(Inquiry, on_delete=models.SET_NULL, null=True)
    ...
    def __str__(self):
        return f"{self.inquiry.customer.name} - " \
    ...
    def save(self, *args, **kwargs):
        ...
        if self.appoint_id is None:
            self.appoint_id = f"{get_timestamp()}{self.appointment_request.id}{generate_random_id()}"
        return super().save(*args, **kwargs)

    def get_client_name(self):
        return self.inquiry.customer.name

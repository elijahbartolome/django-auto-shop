...
from appointment.views import (
    create_appointment, redirect_to_thank_you_page, verify_user_and_login
)
...
class AppointmentRequestTestCase(BaseTest):
    ...
    def test_appointment_request(self):
        ...
        url = reverse('appointment:appointment_request', args=[self.service1.id, self.inquiry.inquiry_id])
...
class VerificationCodeTestCase(BaseTest):
    ...
    def setUp(self):
        ...
        self.url = reverse('appointment:enter_verification_code', args=[self.ar.ar_id, self.ar.inquiry.inquiry_id])
...
class UpdateAppointmentTestCase(BaseTest):
    ...
    def setUp(self):
        ...
        self.data = {
            ...
            'inquiry_id': self.appointment.inquiry.id
        }
    ...
    def test_update_appt_min_info_update(self):
        ...
        self.assertEqual(updated_appt.inquiry.customer.email, self.data['client_email'])
...
class AppointmentDisplayViewTestCase(BaseTest):
    ...
    def setUp(self):
        super().setUp()
        self.appointment = self.create_appt_for_sm1()
        self.url_display_appt = reverse('appointment:display_appointment', args=[self.appointment.appoint_id])
...
class ViewsTestCase(BaseTest):
    ...
    def test_default_thank_you(self):
        ...
        appointment = Appointment.objects.create(appointment_request=self.ar, inquiry=self.ar.inquiry)
        url = reverse('appointment:default_thank_you', args=(appointment.appoint_id,))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
...
class AppointmentClientInformationTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.client = Client()
        self.ar = self.create_appt_request_for_sm1()
        self.url = reverse('appointment:appointment_client_information', args=[self.ar.ar_id])
        self.factory = RequestFactory()
        self.request = self.factory.get('/')
...
class PrepareRescheduleAppointmentViewTests(BaseTest):
    ...
    def setUp(self):
        ...
        self.client = Client()
        self.ar = self.create_appt_request_for_sm1()
        self.url = reverse('appointment:prepare_reschedule_appointment', args=[self.ar.ar_id])
...
class ConfirmRescheduleViewTests(BaseTest):
    ...
    def test_confirm_reschedule_valid(self):
        ...
        self.assertRedirects(response, reverse('appointment:default_thank_you', args=[self.ar.appointment.appoint_id]))
...
class RedirectToPaymentOrThankYouPageTests(BaseTest):
    ...
    def test_redirect_to_custom_thank_you_page(self):
        ...
        response = redirect_to_thank_you_page(self.appointment)

        self.assertIsInstance(response, HttpResponseRedirect)
        self.assertTrue(response.url.startswith(
                reverse('appointment:default_thank_you', kwargs={'appoint_id': self.appointment.appoint_id})))
...
class CreateAppointmentTests(BaseTest):
    ...
    def setUp(self):
        ...
        self.appointment_data = {'phone': '1234567890', 'want_reminder': True,
                                 'additional_info': 'Test info'}
        self.request = RequestFactory().get('/')
        self.inquiry_id = self.inquiry.inquiry_id

    @patch('appointment.views.create_and_save_appointment')
    @patch('appointment.views.redirect_to_thank_you_page')
    ...
    def test_create_appointment_success(self, mock_redirect, mock_create_and_save):
        ...
        create_appointment(self.request, self.appointment_request, self.client_data, self.appointment_data, self.inquiry.inquiry_id)

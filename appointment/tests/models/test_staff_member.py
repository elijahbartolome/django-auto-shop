...
class StaffMemberServiceTests(BaseTest):
    ...
    def test_association_with_multiple_services(self):
        ...
        ori = self.create_service_(name="Ori Shield Configuration", duration=timedelta(hours=1), 
                                   description="Configure the Ori shield")
        symbiote = self.create_service_(name="Symbiote Extraction", duration=timedelta(hours=1), 
    ...
    def test_staff_member_with_non_existent_service(self):
        ...
        with self.assertRaises(ValueError):
            new_staff_member.services_offered.add(
                    Service(id=9999, name="Prometheus Acquisition", duration=timedelta(hours=2)))

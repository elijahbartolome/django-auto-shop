...
from appointment.models import Config, DayOff
...
from appointment.utils.db_helpers import (
    ...
    create_and_save_appointment, create_new_user, day_off_exists_for_date_range,
...
class TestCreateAndSaveAppointment(BaseTest, TestCase):
    ...
    def test_create_and_save_appointment(self):
        client_data = {
            'email': self.inquiry.customer.email,
            'name': self.inquiry.customer.name,
        }
        appointment_data = {
            'phone': self.inquiry.customer.phone_number,
            'want_reminder': True,
            'additional_info': 'Please bring a Zat gun.',
            'inquiry': self.ar.inquiry
        }
        ...
        self.assertEqual(appointment.inquiry.customer.email, client_data['email'])
...
class ScheduleEmailReminderTest(BaseTest):
    ...
    def test_schedule_email_reminder_django_q_available(self, mock_reverse, mock_get_absolute_url, mock_logger,
        ...
        mock_schedule.assert_called_once_with(
                ...
                to_email=self.appointment.inquiry.customer.email,
                name=f"reminder_{self.appointment.id}",
                first_name=self.appointment.inquiry.customer.name,
...
class UpdateAppointmentReminderTest(BaseTest, TestCase):
    ...
    def test_update_appointment_reminder_date_time_changed(self):
        ...
        with patch('appointment.utils.db_helpers.schedule_email_reminder') as mock_schedule_email_reminder, \
            ...
            mock_cancel_existing_reminder.assert_called_once_with(appointment.appoint_id)
...
class CanAppointmentBeRescheduledTests(BaseTest, ConfigMixin):
    ...
    def create_appointment_request_with_histories(self, service, count, use_default_limit=False):
        ar = self.create_appointment_request_(service=service, staff_member=self.staff_member1, inquiry=self.inquiry)
...
class CancelExistingReminderTest(BaseTest):
    def test_cancel_existing_reminder(self):
        if not DJANGO_Q_AVAILABLE:
            return
        appointment = self.create_appt_for_sm1()
        Schedule.objects.create(func='appointment.tasks.send_email_reminder', name=f"reminder_{appointment.appoint_id}")

        self.assertEqual(Schedule.objects.count(), 1)
        cancel_existing_reminder(appointment.appoint_id)
        self.assertEqual(Schedule.objects.filter(name=f"reminder_{appointment.appoint_id}").count(), 0)
...
class TestGetAppointmentByID(BaseTest):
    ...
    def test_existing_appointment(self):
        ...
        fetched_appointment = get_appointment_by_id(self.appointment.appoint_id)
...
class TestGetAppointmentConfigTimes(TestCase):
    ...
    def test_config_object_with_finish_time(self):
        ...
        self.assertEqual(get_appointment_buffer_time(), 60)
        self.assertEqual(get_appointment_lead_time(), (9, 0))
        self.assertEqual(get_appointment_finish_time(), (17, 0))
        self.assertEqual(get_appointment_slot_duration(), 30)
...
class TestGetAppointmentsForDateAndTime(BaseTest):
    ...
    def setUp(self):
        ...
        self.appointment1 = self.create_appointment_(appointment_request=ar1)

        ar2 = self.create_appt_request_for_sm1(start_time=datetime.time(10, 30), end_time=datetime.time(11, 30))
        self.appointment2 = self.create_appointment_(appointment_request=ar2)
        ...
        self.appointment3 = self.create_appointment_(appointment_request=ar3)
...
class TestGetWeekdayNumFromDate(TestCase):
    def test_get_weekday_num_from_date(self):
        ...
        sample_dates = {
            datetime.date(2023, 10, 9): 0,  # Monday
            datetime.date(2023, 10, 10): 1,  # Tuesday
            datetime.date(2023, 10, 11): 2,  # Wednesday
            datetime.date(2023, 10, 12): 3,  # Thursday
            datetime.date(2023, 10, 13): 4,  # Friday
            datetime.date(2023, 10, 14): 5,  # Saturday
            datetime.date(2023, 10, 15): 6,  # Sunday
        }
...
class ExcludePendingReschedulesTests(BaseTest):
    ...
    def test_exclude_with_pending_reschedules_outside_last_5_minutes(self):
        ...
        appointment_request = self.create_appointment_request_(self.service1, self.staff_member1, self.inquiry)

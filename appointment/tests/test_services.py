...
class PrepareAppointmentDisplayDataTests(BaseTest):
    ...
    def test_unauthorized_user(self):
        ...
        x, y, error_message, status_code = prepare_appointment_display_data(self.georges, self.appointment.appoint_id)
    ...
    def test_authorized_user(self):
        ...
        self.assertTrue(self.appointment.inquiry.customer.name in page_title)
...
class SaveAppointmentTests(BaseTest):
    ...
    def test_save_appointment(self):
        ...
        updated_appt = save_appointment(self.appt, client_name, client_email, start_time_str, phone_number,
                                        service_id, self.request, staff_member_id)

        # Check client details
        self.assertEqual(updated_appt.inquiry.customer.name, client_name)
        self.assertEqual(updated_appt.inquiry.customer.email, client_email)
...
class GetAvailableSlotsForStaffTests(BaseTest):
    ...
    def setUp(self):
        ...
        # Staff member1 works only on Mondays and Wednesday (day_of_week: 0, 2)
        self.wh1 = WorkingHours.objects.create(staff_member=self.staff_member1, day_of_week=0,
                                               start_time=datetime.time(9, 0), end_time=datetime.time(17, 0))
        self.wh2 = WorkingHours.objects.create(staff_member=self.staff_member1, day_of_week=2,
                                               start_time=datetime.time(9, 0), end_time=datetime.time(17, 0))
    ...
    def test_booked_slots(self):
        ...
        appt_request = self.create_appointment_request_(service=self.service1, staff_member=self.staff_member1,
                                                        ...
                                                        end_time=end_time, inquiry=self.inquiry)
        # Create an appointment using that request
        self.create_appointment_(appointment_request=appt_request)

...
from appointment.utils.db_helpers import (
    ...
    get_non_working_days_for_staff, get_user_by_email, get_user_model,
...
from .settings import (APPOINTMENT_THANK_YOU_URL)
...
def appointment_request(request, service_id=None, staff_member_id=None, inquiry_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not staff_exists:
                ...
                ar.inquiry = get_object_or_404(Inquiry, inquiry_id=inquiry_id)
                ar.save()
                request.session[f'appointment_completed_{inquiry_id}'] = True
                # Redirect the user to the account creation page
                return redirect("appointment:appointment_client_information", ar_id=ar.ar_id)
        else:
...
def redirect_to_thank_you_page(appointment):
    ...
    thank_you_url = reverse(thank_you_url_key, kwargs={'appoint_id': appointment.appoint_id})
    return HttpResponseRedirect(thank_you_url)


def create_appointment(request, appointment_request_obj, client_data, appointment_data, inquiry_id):
    ...
    notify_admin_about_appointment(appointment, appointment.inquiry.customer.name)
    return redirect_to_thank_you_page(appointment)


def appointment_client_information(request, ar_id):
    ...
    :return: The rendered HTML page.
    """
    ar = get_object_or_404(AppointmentRequest, ar_id=ar_id)
    has_required_email_reminder_config = has_required_email_settings() and check_q_cluster(hide_warning=True)
    appointment = Appointment.objects.filter(inquiry=ar.inquiry).first()

    if appointment:
        context = get_generic_context_with_extra(request, {'ar_id': ar_id}, admin=False)
        return render(request, 'error_pages/304_already_submitted.html', context=context)

    if request.method == 'POST':
        appointment_form = AppointmentForm(request.POST)
        inquiry = get_object_or_404(Inquiry, pk=ar.inquiry.id)

        if appointment_form.is_valid():
            appointment_data = appointment_form.cleaned_data
            appointment_data['phone'] = inquiry.customer.phone_number
            appointment_data['inquiry'] = inquiry
            client_data = {'email': inquiry.customer.email, 'name': inquiry.customer.name}
            ar.save()

            # Create a new appointment
            response = create_appointment(request, ar, client_data, appointment_data, ar.inquiry.id)
            request.session.setdefault(f'appointment_submitted_{ar.inquiry.id}', True)
            return response
    else:
        inquiry = get_object_or_404(Inquiry, pk=ar.inquiry.id)
...
def enter_verification_code(request, ar_id, inquiry_id):
    ...
    if request.method == 'POST':
        ...
        if verify_user_and_login(request, user, code):
            appointment_request_object = AppointmentRequest.objects.get(ar_id=ar_id)
...
def default_thank_you(request, appoint_id):
    ...
    email = ar.inquiry.customer.email
    appointment_details = {
    ...
    send_thank_you_email(ar=ar, email=email, appointment_details=appointment_details,
                         account_details=account_details, request=request, inquiry_id=appointment.inquiry.inquiry_id)
    extra_context = {
...
def prepare_reschedule_appointment(request, ar_id):
    ar = get_object_or_404(AppointmentRequest, ar_id=ar_id)

    if not can_appointment_be_rescheduled(ar):
        url = reverse('appointment:appointment_request', kwargs={'service_id': ar.service.id, 'inquiry_id': ar.inquiry.inquiry_id})
        context = get_generic_context_with_extra(request, {'url': url, }, admin=False)
        logger.error(f"Appointment with id_request {ar_id} cannot be rescheduled")
...
def reschedule_appointment_submit(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            client_first_name = ar.inquiry.customer.name
            email = ar.inquiry.customer.email
...
def confirm_reschedule(request, id_request):
    ...
    client_name = ar.inquiry.customer.name
    notify_admin_about_reschedule(reschedule_history, ar, client_name)
    return redirect('appointment:default_thank_you', appoint_id=ar.appointment.appoint_id)

...
Inquiry = apps.get_model('customers', 'Inquiry')
...
def schedule_email_reminder(appointment, request, appointment_datetime=None):
    ...
    ar_id = appointment.appointment_request.ar_id
    relative_reschedule_url = reverse('appointment:prepare_reschedule_appointment', args=[ar_id])
    ...
    schedule('appointment.tasks.send_email_reminder',
             to_email=appointment.inquiry.customer.email,
             name=f"reminder_{appointment.id}",
             first_name=appointment.inquiry.customer.name,
...
def get_appointment_by_id(appoint_id):
    ...
    """
    try:
        return Appointment.objects.get(appoint_id=appoint_id)
...
def get_appointment_finish_time():
    ...
    if config and config.finish_time:
        return config.finish_time.hour, config.finish_time.minute
...
def get_appointment_lead_time():
    ...
    if config and config.lead_time:
        return config.lead_time.hour, config.lead_time.minute
...
def get_times_from_config(date):
    ...
    """
    start_hour, start_minute = get_appointment_lead_time()
    start_time = datetime.datetime.combine(date, datetime.time(hour=start_hour, minute=start_minute))
    finish_hour, finish_minute = get_appointment_finish_time()
    end_time = datetime.datetime.combine(date, datetime.time(hour=finish_hour, minute=finish_minute))
    slot_duration = datetime.timedelta(minutes=get_appointment_slot_duration())
    buff_time = datetime.timedelta(minutes=get_appointment_buffer_time())
